...
namespace DXRFramework
{
    ...
    ID3D12RaytracingFallbackStateObject *RtState::getFallbackRtso()
    {
        ...
        for (int i = 0; i < mProgram->getHitProgramCount(); ++i) {
            ...
            std::wstring anyHitSymbol = converter.from_bytes(hitGroup.mClosestHit->mEntryPoint);
            std::wstring intersectionSymbol = converter.from_bytes(hitGroup.mClosestHit->mEntryPoint);
