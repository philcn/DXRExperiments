...
CameraController::CameraController( Camera& camera, Vector3 worldUp ) : m_TargetCamera( camera )
{
    ...
    m_MoveSpeed = 2.0f;
    m_StrafeSpeed = 2.0f;
    m_MouseSensitivityX = 0.6f;
    m_MouseSensitivityY = 0.6f;
...
void CameraController::Update( float deltaTime )
{
    ...
    if (GameInput::IsFirstPressed(GameInput::kRThumbClick) || GameInput::IsFirstPressed(GameInput::kKey_lshift))
        m_FineRotation = !m_FineRotation;

    float speedScale = (m_FineMovement ? 0.2f : 1.0f) * timeScale;
